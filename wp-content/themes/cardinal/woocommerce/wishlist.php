...
<div class="my-account-left">
    ...
    <ul class="nav my-account-nav">
        <?php if ( is_user_logged_in() ) { ?>
            <li><a href="<?php echo $myaccount_page_url; ?>"><?php _e( "Back to my account", "swiftframework" ); ?></a>
            </li>
        <?php } else { ?>
            <li><a href="<?php echo $shop_page_url; ?>"><?php _e( "Shop", "swiftframework" ); ?></a></li>
            <li>
                <a href="<?php echo $myaccount_page_url; ?>"><?php _e( "Create Account / Login", "swiftframework" ); ?></a>
            </li>
        <?php } ?>
    </ul>
        ...
        <table class="shop_table cart wishlist_table" cellspacing="0">
            ...
            <tr>
                ...
                <?php if ( get_option( 'yith_wcwl_price_show' ) == 'yes' ) { ?>
                    <th class="product-price"><span class="nobr"><?php _e( 'Unit Price', 'swiftframework' ) ?></span>
                    </th><?php } ?>
                <?php if ( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) { ?>
                    <th class="product-stock-status"><span
                        class="nobr"><?php _e( 'Stock Status', 'swiftframework' ) ?></span></th><?php } ?>
                <?php if ( ! $sf_catalog_mode && get_option( 'yith_wcwl_add_to_cart_show' ) == 'yes' ) { ?>
                    <th class="product-add-to-bag"><span class="nobr"><?php _e( 'Actions', 'swiftframework' ) ?></span>
                    </th>
                <?php } ?>
            ...
            <?php
                if ( count( $wishlist ) > 0 ) :
                            ...
                            <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                                ...
                                <td class="product-remove">
                                    <div><a href="javascript:void(0)"
                                            ...
                                            class="remove"
                                            title="<?php _e( 'Remove this product', 'swiftframework' ) ?>"><i
                                                class="ss-delete"></i></a>
                                </td>
                            </tr>
                        <?php
                    ...
                    <tr>
                        <td colspan="6"
                            class="wishlist-empty"><?php _e( 'Your wishlist is currently empty.', 'swiftframework' ) ?></td>
                    </tr>
                <?php
                endif;
