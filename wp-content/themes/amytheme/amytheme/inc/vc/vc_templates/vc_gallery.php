<?php
$output = $title = $type = $onclick = $custom_links = $img_size = $img_color = $custom_links_target = $images = $el_class = $interval = '';
...
if ( $type == 'nivo' ) {
    ...
    $el_start = '<li><div class="imghoverfxbg grid "><div class="slelementno"><figure>';
    $el_end = '</figure></div></div></li>';
  ...
  //  wp_enqueue_script('flexslider');
    ...
    $el_start = '<li class="isotope-item"><div class="imghoverfxbg grid '.$img_color.'"><div class="slelementno"><figure>';
    $el_end = '</figure></div></div></li>';
...
$pretty_rel_random = ' rel="prettyPhotoImages[rel-'.rand().']"'; //rel-'.rand();
...
foreach ( $images as $attach_id ) { 
    ...
    $link_start = $link_end = '';
	if ( $onclick == 'link_image' ) {
		$gal_images .= $el_start . $thumbnail . '<figcaption><a '.$pretty_rel_random.' href="'.$p_img_large[0].'"'.$pretty_rel_random.'>
					<div class="clickimg">
						<i class="icon-search"></i>
					</div>
				</a></figcaption>' . $el_end;
	}else if (  $onclick == 'custom_link' && isset( $custom_links[$i] ) && $custom_links[$i] != '' ) {
		$gal_images .= $el_start . $thumbnail . '<figcaption><a href="'.$custom_links[$i].'"' . (!empty($custom_links_target) ? ' target="'.$custom_links_target.'"' : '') . '>
					<div class="clickimg">
						<i class="icon-link"></i>
					</div>
				</a></figcaption>' . $el_end;
...
$output .= "\n\t".'<div class="'.$css_class.'">';
$output .= "\n\t\t".'<div class="wpb_wrapper '.$img_color.'">';
