 ...
 */
function sopernal_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'sopernal_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'sopernal' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => sopernal_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'sopernal' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'sopernal' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'sopernal' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'sopernal' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'sopernal' );
}
add_action( 'customize_register', 'sopernal_customize_register', 11 );

/**
 * Register color schemes for Twenty Fifteen.
 *
 * Can be filtered with {@see 'sopernal_color_schemes'}.
 *
 ...
 */
function sopernal_get_color_schemes() {
	return apply_filters( 'sopernal_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'sopernal' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'sopernal_get_color_scheme' ) ) :
/**
 ...
 */
function sopernal_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = sopernal_get_color_schemes();
...
endif; // sopernal_get_color_scheme

if ( ! function_exists( 'sopernal_get_color_scheme_choices' ) ) :
/**
 ...
 */
function sopernal_get_color_scheme_choices() {
	$color_schemes                = sopernal_get_color_schemes();
...
endif; // sopernal_get_color_scheme_choices

if ( ! function_exists( 'sopernal_sanitize_color_scheme' ) ) :
/**
 ...
 */
function sopernal_sanitize_color_scheme( $value ) {
	$color_schemes = sopernal_get_color_scheme_choices();
...
endif; // sopernal_sanitize_color_scheme
 ...
 */
function sopernal_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = sopernal_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = sopernal_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = sopernal_get_color_scheme_css( $colors );

	wp_add_inline_style( 'sopernal-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'sopernal_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function sopernal_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', sopernal_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'sopernal_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function sopernal_customize_preview_js() {
	wp_enqueue_script( 'sopernal-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'sopernal_customize_preview_js' );
 ...
 */
function sopernal_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function sopernal_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-sopernal-color-scheme">
		<?php echo sopernal_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'sopernal_color_scheme_css_template' );
